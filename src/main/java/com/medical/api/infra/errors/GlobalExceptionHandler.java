...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiErrorResponse> handleAuthenticationError(AuthenticationException ex, HttpServletRequest request) {
    ...
    public ResponseEntity<ApiErrorResponse> handleInvalidJson(HttpMessageNotReadableException ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.BAD_REQUEST, "Malformed JSON Request", "Check your JSON structure or Enum values", request);
    }
    ...
    public ResponseEntity<ApiErrorResponse> handleConflict(DataIntegrityViolationException ex, HttpServletRequest request) {
        return buildResponse(HttpStatus.CONFLICT, "Data Integrity Violation", "Duplicate entry or constraint violation", request);
    }
    ...
    private ResponseEntity<ApiErrorResponse> buildResponse(HttpStatus status, String title, String detail, HttpServletRequest request) {
        var error = new ApiErrorResponse(
                title,
                status.value(),
                LocalDateTime.now(),
